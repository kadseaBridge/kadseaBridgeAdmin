...
// 生成路径: gfast/app/work/service/bridge_config.go
// 生成人：gfast
// ==========================================================================
...
func (s *bridgeConfig) GetList(req *dao.BridgeConfigSearchReq) (total, page int, list []*model.BridgeConfig, err error) {
	m := dao.BridgeConfig.Ctx(req.Ctx)     
        ...
        if req.TargetChainId != "" {
            m = m.Where(dao.BridgeConfig.Columns.TargetChainId+" = ?", req.TargetChainId)
        }     
        if req.SourceCoinAddress != "" {
            m = m.Where(dao.BridgeConfig.Columns.SourceCoinAddress+" = ?", req.SourceCoinAddress)
        }     
        if req.IsEnable != "" {
            m = m.Where(dao.BridgeConfig.Columns.IsEnable+" = ?", req.IsEnable)
        }     
        if req.TargetCoinAddress != "" {
            m = m.Where(dao.BridgeConfig.Columns.TargetCoinAddress+" = ?", req.TargetCoinAddress)
        }
	total, err = m.Count()
	if err != nil {
		g.Log().Error(err)
