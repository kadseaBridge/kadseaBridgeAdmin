...
// 生成路径: adminBackend/app/admin/service/coin.go
// 生成人：jimmy
// ==========================================================================
...
// GetInfoById 通过id获取
func (s *coin) GetAddressByNameAndChainId(ctx context.Context, symbol string, chainId string) (address string, err error) {
	...
	// 只选择 Name 字段
	err = dao.Coin.Ctx(ctx).Where("symbol = ? AND chainId = ?", symbol, chainId).Fields("address").Scan(&result)
	if err != nil {
		g.Log().Error(err)
		return
	}
...
// GetInfoById 通过id获取
func (s *coin) GetAddressBySymbol(ctx context.Context, symbol string) (addressList []string, err error) {
	if symbol == "" {
		err = gerror.New("参数错误")
		return
	}
	...
	// 只选择 Name 字段
	//err = dao.Coin.Ctx(ctx).Where(dao.Coin.Columns.Symbol, symbol).Fields("address").Scan(&results)
	err = dao.Coin.Ctx(ctx).Where(dao.Coin.Columns.Symbol+" LIKE ?", "%"+symbol+"%").Fields("address").Scan(&results)
	if err != nil {
		g.Log().Error(err)
		return
	}
	if len(results) == 0 {
		err = gerror.New("获取address失败")
		return
	}

	for _, result := range results {
		addressList = append(addressList, result.Address)
	}
	return
}
